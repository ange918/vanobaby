...
const tickets = [
  {
    label: 'PELOUSE',
    price: '15 000 FCFA',
    perks: ['Accès zone principale', 'Animation pré-show', 'Stationnement gratuit', 'Badge souvenir'],
    featured: false,
  },
  {
    label: 'TRIBUNE',
    price: '30 000 FCFA',
    perks: ['Places assises tribunes', 'Vue panoramique scène', 'Boisson incluse', 'Accès prioritaire'],
    featured: false,
  },
  {
    label: 'VIP',
    price: '75 000 FCFA',
    perks: ['Zone VIP exclusive', 'Meet & Greet artiste', 'Open bar premium', 'Kit collector signé'],
    featured: true,
  },
];
...
const faqItems = [
  ...
  { q: 'Y a-t-il un parking disponible ?', a: 'Oui, un parking sécurisé et gratuit est disponible pour tous les billets PELOUSE et au-dessus. Le co-voiturage est encouragé.' },
    ...
    >
      ...
      >
        ...
        <motion.span
          ...
          style={{ fontSize: '1.5rem', color: '#C0392B', flexShrink: 0, lineHeight: 1 }}
        >
          +
        </motion.span>
      </button>
      <AnimatePresence>
        {open && (
          ...
          >
            <p style={{ color: '#999999', fontSize: '0.85rem', lineHeight: 1.8, paddingBottom: '1.25rem' }}>
              {item.a}
            </p>
          </motion.div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {tickets.map((ticket, i) => (
              ...
              >
                ...
                <p style={{ fontFamily: 'var(--font-bebas)', fontSize: '2rem', color: '#ffffff', letterSpacing: '0.06em', marginBottom: '0.5rem', marginTop: ticket.featured ? '1rem' : '0' }}>
                  {ticket.label}
                </p>
                <p style={{ fontFamily: 'var(--font-bebas)', fontSize: 'clamp(1.4rem, 3vw, 2rem)', color: '#C0392B', marginBottom: '1.5rem' }}>
                  {ticket.price}
                </p>
                <ul style={{ listStyle: 'none', padding: 0, marginBottom: '2rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  {ticket.perks.map(perk => (
                    <li key={perk} style={{ color: '#999999', fontSize: '0.82rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <span style={{ color: '#C0392B', fontWeight: 700 }}>—</span> {perk}
                    </li>
                  ))}
                </ul>
                <a
